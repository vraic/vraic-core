...
- **Example:**
  ...
  travel_to Time.zone.local(2024, 1, 1, 12, 0, 0) do
    fill_in "otp_code", with: @user.current_otp
    click_button "Verify"
  end
  ```
...
- **Best Practices:**
  ...
  - **Actionable Elements:** Prefer `click_button` or `click_on` over generic `find().click` to benefit from Capybara's built-in waiting and visibility checks.
